...
namespace Foog.Core.FreeSql
{
    ...
    public class KeyRepository<TEntity, TKey> : BaseRepository<TEntity, TKey>, IKeyRepository<TEntity, TKey>
        where TEntity : EntityBase<TKey>, new()
    {

        public readonly IFreeSql _fsql;

        public KeyRepository(IFreeSql fsql, UnitOfWorkManager uowManger) : base(uowManger?.Orm ?? fsql, null, null)
        {
            //事务管理绑定
